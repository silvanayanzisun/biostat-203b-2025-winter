...
server <- function(input, output, session) {
  ...
  patient_data <- eventReactive(input$submit_patient, {
    ...
    # filter
    patient_info <- filter(subject_id == patient_id) |> collect()
    transfers <- filter(subject_id == patient_id) |> collect()
    labs <- filter(subject_id == patient_id) |> collect()
    procedures <- filter(subject_id == patient_id) |> collect()
    diagnoses <- filter(subject_id == patient_id) |> collect()
...
procedure_data <- procedure_data  |>
  left_join(d_icd_procedures_df, by = "icd_code") 
    ...
    ggplot() +
      geom_segment(data = transfers, aes(x = as.Date(intime), xend = as.Date(outtime), 
                                         y = "ADT", yend = "ADT", color = careunit), size = 3) +
      geom_point(data = labs, aes(x = as.Date(charttime), y = "Lab"), shape = 3, size = 2) +
      geom_point(data = procedures, aes(x = as.Date(chartdate), y = "Procedure", shape = long_title), size = 3) +
      theme_minimal()
  })
}
shinyApp(ui, server)