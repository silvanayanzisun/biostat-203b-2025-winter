...
chartevents_tble <- open_dataset("chartevents_parquet", format = "parquet") |>
  ...
  #slice_max(mean_valuenum, n = 1) |>
  ungroup() |>
  pivot_wider(names_from = itemid, values_from = mean_valuenum) |>
  rename_at (
...
```{r}
ggplot(mimic_icu_cohort, aes(x=c(race, insurance, marital_status, gender, anchor_age), y=length_icu_stay))
```
