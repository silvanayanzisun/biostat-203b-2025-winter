...
**Solution:**here is the answer to question 2. The files are gzip-compressed csv files so they has .gz at the end of file name. Compressing files can save storage space.
```{bash}
ls -l ~/mimic/hosp
ls -l ~/mimic/icu
```
...
**Solution:**zcat allows to view a compressed (zipped) file directly, basically a cat command for zipped files.
zless view compressed file contents in a paginated way, can also do search option
zmore view compressed file contents in a new tab
zgrep search string/pattern in the compressed file
...
**Solution:** 
There are 546028 rows in this file excluding the headerline. There are 546028 hospitalizations in this data file. There are 223452 unique patients, which does not matchthe number of patients listed in the patients.csv(364627)
...
**Solution:** 
Redirection >: redirecting the standard output to a file
> save text to output file (overwrite if file exist) 
...
echo {con,pre}{sent,fer}{s,ed} prints out all possible combinations of the characters in curly brackets. 2*2*2=8
...
I cloned the repository and compiled the book. here is the screenshot of 4.1.5 of the book.
![yanzisun screenshot of 4.1.5](/Users/yanzisun/Desktop/203B/203B-HW/HW1/screenshot hw1.jpg)